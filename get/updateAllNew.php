...
$client->setAuthConfig('/home/baumuin/public_html/battle/get/client_credentials.json');
$client->setAccessType ("offline");
$client->setApprovalPrompt ("force");
$client->setIncludeGrantedScopes(true);
$client->addScope("https://www.googleapis.com/auth/photoslibrary.readonly");

$accessFile = '/home/baumuin/public_html/battle/get/accessToken.json';
$refreshFile = '/home/baumuin/public_html/battle/get/refreshToken.json';
...
for ($i = 0; $i < sizeof($albums); $i++){
	...
	if(mysqli_num_rows($irnav) == 0 ) {
		$pictures = showAlbumContent($service, $albumID);
		
		
		if(is_array($pictures)){
			echo "Sākam albumu <b>".$albums[$i][1]."</b> (sauksim to par <i>".$FullTitle."</i>, tā ID ir ".$albumID.")<br/>";
			//iet cauri bildēm
			for ($j=0;$j<count($pictures);$j++){
				...
				//pievieno jaunu
				$result = mysqli_query($connection, "insert into ratings (img, album, albumID, iso, model, fstop, exposure, focallength, year, month, day, taken, views) values('$random_pic', '$FullTitle', '$albumID', '$iso', '$model', '$fstop', '$exposure', '$focallength', '$year', '$month', '$weekday', '$taken', NULL)");
				echo "Albums <b>".$origTitle."</b> (".$FullTitle.") papildināts ar bildi ";
                echo "<img style='height:20px;' onload='(function(){var imgElement = this; var jsonURL=\"https://photoslibrary.googleapis.com/v1/mediaItems/".$random_pic."?access_token=".$accessToken."\"; $.getJSON(jsonURL, function(data) { var imgURL = data.baseUrl+\"=w2000\"; imgElement.src=imgURL; }); }).call(this)' src='../includes/bigLoader.gif'/><br/>";
			}
		}
	}
}
